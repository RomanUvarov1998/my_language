...
impl DataTypeTemplate {
	...
	pub fn generate_type_def(&self, type_params: &HashMap<String, String>, context: &Context) -> Result<StructDef, InterpErr> {
		...
		for (f_name, f) in self.fields.iter() {
			let dt: DataType = context.find_type_by_name(&f.data_type_name)?;
			let name_nt = NameToken::new_with_pos(
				f_name.to_string(),
				CodePos::from(CharPos::new()),
				true);
		...
		let mut sd = StructDef::new(name_nt, fields)?;
...
impl UserFuncDefTemplate {
	fn generate(&self, type_params: &HashMap<String, String>, context: &Context) -> Result<UserFuncDef, InterpErr> {
		let mut args: Vec<UserFuncArg> = Self::resolve_user_member_function_args(type_params, &self.args, context)?;
		
		let return_type: DataType = context.find_type_by_name(&self.return_type_name)?;
	...
	fn resolve_statements(template_statements: &Vec<Statement>, type_params: &HashMap<String, String>, next_context: &mut Context) -> Result<Vec<Statement>, InterpErr> {
		...
		for st in template_statements.iter() {
			let st_kind: StatementKind = match st.kind() {
				...
				StatementKind::UserStructDeclare { new_type_name, .. } => {
					return Err( StructDefErr::StructDefNotInRootContext {
						struct_pos: new_type_name.pos(),
					}.into() );
				},
				...
				StatementKind::UserDefinedFuncDeclare { name, args, return_type_name, body } => {
					let (resolved_return_type_name, return_type): (Option<NameToken>, DataType) = match return_type_name {
						Some(tn) => {
							...
							let return_type: DataType = next_context.find_type_by_name(&resolved_tn)?;
	...
	fn resolve_function_parsed_args(type_params: &HashMap<String, String>, template_args: &Vec<ParsedFuncArgDef>, context: &Context) -> Result<Vec<ParsedFuncArgDef>, InterpErr> {
	...
	fn resolve_user_member_function_args(type_params: &HashMap<String, String>, template_args: &Vec<ParsedFuncArgDef>, context: &Context) -> Result<Vec<UserFuncArg>, InterpErr> {
		let mut args = Vec::<UserFuncArg>::new();
		
		for temp_arg in template_args.iter() {
			let arg_type_name: NameToken = Self::resolve_type_name(type_params, temp_arg.data_type_name(), context)?;
			let arg_data_type: DataType = context.find_type_by_name(&arg_type_name)?;
	...
	fn resolve_expr(type_params: &HashMap<String, String>, expr: &Expr, context: &Context) -> Result<Expr, InterpErr> {
		...
		Ok( Expr::new_from_stack(resolved_expr_stack) )
	}
	
	fn resolve_conditional_body(type_params: &HashMap<String, String>, cb: &ConditionalBody, context: &Context) -> Result<ConditionalBody, InterpErr> {
		...
		Ok( ConditionalBody::new(rce, rsts) )
	}	
	
	fn resolve_unconditional_body(type_params: &HashMap<String, String>, ucb: &UnconditionalBody, context: &Context) -> Result<UnconditionalBody, InterpErr> {
		let mut next_context = context.new_stack_frame_context();
		let rsts: Vec<Statement> = Self::resolve_statements(ucb.statements(), type_params, &mut next_context)?;
		Ok( UnconditionalBody::new(rsts) )
	}
	
	fn resolve_returning_body(type_params: &HashMap<String, String>, statements: &Vec<Statement>, return_type: &DataType, context: &Context) -> Result<ReturningBody, InterpErr> {
		let mut next_context = context.new_stack_frame_context();
		...
		ret_body.check(return_type, &mut next_context)?;
		Ok(ret_body)
	}
	
	fn resolve_type_name(type_params: &HashMap<String, String>, type_name: &NameToken, context: &Context) -> Result<NameToken, InterpErr> {
		if let Ok(_) = context.find_type_by_name(type_name) {
			Ok(type_name.clone())
		} else if let Some(new_name) = type_params.get(type_name.value()) {
			let nt = NameToken::new_with_pos(
				new_name.clone(),
				CodePos::from(CharPos::new()),
				true);				
			Ok(nt)
		} else {
			Err( TemplateErr::NotSatisfiedTypeParam { name: type_name.clone() }.into() )
		}
	}
}
